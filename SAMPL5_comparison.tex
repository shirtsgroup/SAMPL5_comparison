...
\title{Lesson learned from comparison of molecular dynamics engines on the SAMPL5 dataset}
...
simulations are run with program-specific default simulation parameter values. 
...
The first task that a molecular simulation engine has is to take a
...
It is always challenging to obtain comparable results for the same
inputs in two different simulation engines, because the input
...
GROmacs~\citep{topogromacs}, CHARMM-GUI~\citep{lee_charmm-gui_2016}
which can convert from CHARMM to GROMACS (and uses ParmEd to
manipulate AMBER files), and{\tt amber2lmp}, a script converting
between LAMMPS and AMBER files. 
...
host. Specific details of the topology construction and configuration
generation is described in the SAMPL5 overview.
paper.~\cite{mainSAMPL5paper}
...
\item {\bf LAMMPS}: Energies were calculated with the April 5,
  ...
  %CTK: Which ones specifically? Just RIGID for constraints, correct?
...
\end{tabulary}
\end{table}
%CTK: Not critical to the point being made but might want to mention which
%system the above data come from.
...
\end{table}
%CTK: I took a whack at improving the formatting. Replacing 48x48x48 with
%48^3 might be somewhat contentious though...
